...
and reloc_expr floc sh =
  self where rec self =
    ...
    | ExJdf loc x1 x2 →
        let loc = floc loc in
        ExJdf loc (vala_map (List.map (reloc_joinclause floc sh)) x1) (self x2)
    | ExLab loc x1 →
        let loc = floc loc in
        ExLab loc
          (vala_map
...
and reloc_str_item floc sh =
  self where rec self =
    ...
    | StDef loc x1 →
        let loc = floc loc in
        StDef loc (vala_map (List.map (reloc_joinclause floc sh)) x1)
    | StDir loc x1 x2 →
        let loc = floc loc in
...
and reloc_joinclause floc sh x =
  {jcLoc = floc x.jcLoc;
   jcVal =
     vala_map
       (List.map
                ...
                (List.map
                   (fun (loc, x1, x2) →
                      (floc loc, (fun (loc, x1) → (floc loc, x1)) x1,
                       vala_map (option_map (reloc_patt floc sh)) x2)))
                x1,
              reloc_expr floc sh x2)))
       x.jcVal}
