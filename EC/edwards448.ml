...
let EDWARD_NONSINGULAR_448 = prove
...
let EDWARDS448_GROUP = prove
 (`group_carrier edwards448_group =
  ...
  REWRITE_TAC[EDWARD_NONSINGULAR_448] THEN
...
let EDWARDS448_ORDER_2 = prove
 (`{p | p IN group_carrier edwards448_group /\
  ...
  W(MP_TAC o PART_MATCH (lhand o rand)  EDWARDS_GROUP_ORDER_EQ_2 o
    lhand o snd) THEN
  ANTS_TAC THENL
   [REWRITE_TAC[EDWARD_NONSINGULAR_448] THEN
    REWRITE_TAC[FIELD_INTEGER_MOD_RING; PRIME_P448] THEN
...
let EDWARDS448_ORDER_8 = prove
 (`{p | p IN group_carrier edwards448_group /\
  ...
  REWRITE_TAC[edwards448_group] THEN
  W(MP_TAC o PART_MATCH (lhand o rand)  EDWARDS_GROUP_ORDER_EQ_8_EQUIV o
    rand o snd) THEN
  ANTS_TAC THENL
   [REWRITE_TAC[EDWARD_NONSINGULAR_448] THEN
    REWRITE_TAC[FIELD_INTEGER_MOD_RING; PRIME_P448] THEN
