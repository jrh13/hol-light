...
let GL_truth_lemma = prove
 (`!W R p V q.
  ...
  REPEAT GEN_TAC THEN REWRITE_TAC[GL_STANDARD_MODEL; GL_STANDARD_FRAME] THEN
...
let EXTEND_MAXIMAL_CONSISTENT = prove
 (`!p X. CONSISTENT X /\
  ...
  [ASM_REWRITE_TAC[NOREPETITION_CLAUSES] THEN
   ...
   REWRITE_TAC[MEM] THEN ASM_MESON_TAC[SUBLIST];
   ALL_TAC] THEN
  INTRO_TAC "@M. max sub" THEN EXISTS_TAC `M:form list` THEN
  ASM_REWRITE_TAC[] THEN REMOVE_THEN "sub" MP_TAC THEN
  REWRITE_TAC[SUBLIST; MEM] THEN MESON_TAC[]);;
 ...
 (`!p. ~ |-- p
  ...
  [CONJ_TAC THENL
   [REWRITE_TAC[CONSISTENT_SING] THEN ASM_MESON_TAC[GL_DOUBLENEG_CL];
    GEN_TAC THEN REWRITE_TAC[MEM] THEN MESON_TAC[SUBFORMULA_REFL]];
   STRIP_TAC THEN EXISTS_TAC `M:form list` THEN
   ASM_MESON_TAC[SUBLIST; MEM]]);;
...
let GL_STANDARD_REL = new_definition
  `GL_STANDARD_REL p w x <=>
   ...
   (!q. MEM q w ==> q SUBFORMULA p \/ ?q'. q = Not q' /\ q' SUBFORMULA p) /\
   MAXIMAL_CONSISTENT p x /\
   (!q. MEM q x ==> q SUBFORMULA p \/ ?q'. q = Not q' /\ q' SUBFORMULA p) /\
 ...
 (`!p. ~ |-- p
  ...
  CONJ_TAC THENL [
   ...
   EXISTS_TAC
     `{l | NOREPETITION l /\
           !q. MEM q l
               ==> q IN {q | q SUBFORMULA p \/
                             (?q'. q = Not q' /\ q' SUBFORMULA p)}}` THEN
    CONJ_TAC THENL
    [MATCH_MP_TAC FINITE_NOREPETITION THEN
     POP_ASSUM_LIST (K ALL_TAC) THEN
     SUBGOAL_THEN
       `{q | q SUBFORMULA p \/ (?q'. q = Not q' /\ q' SUBFORMULA p)} =
     ...
     [REWRITE_TAC[GSYM SUBSET_ANTISYM_EQ; SUBSET; FORALL_IN_UNION;
      ...
      ASM_REWRITE_TAC[injectivity "form"; UNWIND_THM1] THEN
      ASM_SIMP_TAC[IN_ELIM_THM; FUN_IN_IMAGE];
      ALL_TAC] THEN
...
let ACCESSIBILITY_LEMMA =
 ...
 (`!p M w q.
  ...
  [ASM_REWRITE_TAC[MEM] THEN GEN_TAC THEN STRIP_TAC THEN
   ...
   ASM_REWRITE_TAC[distinctness "form"; injectivity "form"] THENL
   [DISJ2_TAC THEN REWRITE_TAC[UNWIND_THM1] THEN HYP MESON_TAC "boxq"
    ...
    [MATCH_MP_TAC SUBFORMULA_TRANS THEN EXISTS_TAC `Box a` THEN CONJ_TAC THENL
     [REWRITE_TAC[SUBFORMULA_INVERSION; SUBFORMULA_REFL]; ALL_TAC] THEN
     REMOVE_THEN "subw" (MP_TAC o SPEC `Box a`) THEN
     ASM_REWRITE_TAC[distinctness "form"];
     MATCH_MP_TAC SUBFORMULA_TRANS THEN EXISTS_TAC `Box a` THEN CONJ_TAC THENL
     [REWRITE_TAC[SUBFORMULA_INVERSION; SUBFORMULA_REFL]; ALL_TAC] THEN
     REMOVE_THEN "subw" (MP_TAC o SPEC `Box a`) THEN
     ASM_REWRITE_TAC[distinctness "form"]]];
