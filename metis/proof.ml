...
type inference =
  | Axiom of Literal.literal Mset.set
  | Assume of Atom.atom
...
let reconstructSubst cl cl' =
  let rec recon = function
    ...
    | (lit :: lits, sub) :: others ->
        ...
        recon (Literal.Set.foldl checkLit others cl') in
...
let reconstructResolvant cl1 cl2 cl =
  if not (Literal.Set.subset cl1 cl) then
    ...
    and cl2' = Literal.Set.negate cl2 in
    let lits = Literal.Set.intersectList [cl1;cl1';cl2;cl2'] in
...
let reconstructEquality cl =
  ...
  let candidates =
    ...
    | _ -> raise (Bug "reconstructEquality: malformed") in
...
let reconstruct cl = function
  ...
  | (Thm.Resolve,[th1;th2]) ->
      ...
      and cl2 = Thm.clause th2 in
      let (pol,atm) = reconstructResolvant cl1 cl2 cl in
  ...
  | (Thm.Equality,[]) ->
      let (x,y,z) = (reconstructEquality cl) in
      Equality (x,y,z)
  | _ -> raise (Bug "malformed inference");;

let thmToInference th =
    let cl = Thm.clause th in
    let thmInf = Thm.inference th in
    let inf = reconstruct cl thmInf in
    inf
;;
...
let proof th =
  let emptyThms : (Literal.literal Mset.set, Thm.thm) Mmap.map =
    Literal.Set_map.newMap () in
  let rec addThms (th,ths) =
    let cl = Thm.clause th in
    if Literal.Set_map.inDomain cl ths then ths
    else
      let (_,pars) = Thm.inference th in
      let ths = List.foldl (curry addThms) ths pars in
      if Literal.Set_map.inDomain cl ths then ths
      else Literal.Set_map.insert ths (cl,th) in
  let mkThms th = addThms (th,emptyThms) in
  let rec addProof (th,(ths,acc)) =
    let cl = Thm.clause th in
    match Literal.Set_map.peek ths cl with
    | None -> (ths,acc)
    | Some th ->
        let (_,pars) = Thm.inference th in
        let (ths,acc) = List.foldl (curry addProof) (ths,acc) pars in
        let ths = Literal.Set_map.delete ths cl in
        let acc = (th, thmToInference th) :: acc in
        (ths,acc) in
  let mkProof ths th =
    let (ths,acc) = addProof (th,(ths,[])) in
    List.rev acc in
  let ths = mkThms th in
  let infs = mkProof ths th in
  infs
;;
...
let freeIn v =
  let free th_inf =
    ...
    | (_, Equality (lit,_,tm)) ->
        Literal.freeIn v lit || Term.freeIn v tm in
  List.exists free
;;
  ...
  let inc (th_inf,set) =
    Name.Set.union set
      (match th_inf with
       ...
       | (_, Equality (lit,_,tm)) ->
         Name.Set.union (Literal.freeVars lit) (Term.freeVars tm)) in
  List.foldl (curry inc) Name.Set.empty
;;
