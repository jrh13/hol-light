...
type priority = Word64.word;;

let randomPriority = randomWord;;

let comparePriority = fun x y ->
  Int.compare (Word64.toInt x) (Word64.toInt y);;
...
let lowerPriorityNode node1 node2 =
  let p1 = node1.Node.priority
  and p2 = node2.Node.priority in
...
let mkNode priority left key value right =
  let size = treeSize left + 1 + treeSize right in
    Node {
      size = size;
      priority = priority;
      left = left;
...
let mkTree priority left key value right =
  let node = mkNode priority left key value right in
  Tree node
;;
...
let nodeSingleton (key,value) =
  let priority = randomPriority () in
  mkNodeSingleton priority key value
;;

let treeSingleton key_value =
  let node = nodeSingleton key_value in
  Tree node
;;
  ...
  | Tree node1 ->
      ...
      | Tree node2 ->
          if lowerPriorityNode node1 node2 then
            let Node {priority; left; key; value; right} = node2 in
            let left = treeAppend tree1 left in
            mkTree priority left key value right
          else
            let Node {priority; left; key; value; right} = node1 in
            let right = treeAppend right tree2 in
            mkTree priority left key value right
;;
...
let treeCombine left node right =
  let left_node = treeAppend left (Tree node) in
...
and nodePeek compareKey pkey node =
  let Node {left; key; value; right} = node in
...
and nodePeekPath compareKey pkey path node =
  let Node {left; key; right} = node in
  match compareKey pkey key with
...
let addSidePath (wentLeft,node) (leftTree,rightTree) =
  let Node {priority; left; key; value; right} = node in
...
let addSidesPath left_right = List.foldl addSidePath left_right;;
...
let updateTreePath tree = List.foldl updateTree tree;;
...
let insertNodePath node =
  let rec insert left_right path =
    match path with
    | [] ->
        let (left,right) = left_right in
          treeCombine left node right
    | ((_,snode) as step) :: rest ->
        if lowerPriorityNode snode node then
          let left_right = addSidePath step left_right in
          insert left_right rest
        else
          let (left,right) = left_right in
          let tree = treeCombine left node right in
          updateTreePath tree path in
  insert (Empty,Empty)
;;
...
let nodePartition compareKey pkey node =
  let (path,pnode) = nodePeekPath compareKey pkey [] node in
  match pnode with
  | None ->
      let (left,right) = mkSidesPath path in
      (left,None,right)
  | Some node ->
      let Node {left; key; value; right} = node in
      let (left,right) = addSidesPath (left,right) path in
...
let treeInsert compareKey key_value tree =
  ...
  let (path,inode) = treePeekPath compareKey key [] tree in
  match inode with
  | None ->
      let node = nodeSingleton (key,value) in
      insertNodePath node path
  | Some node ->
      let Node {size; priority; left; right} = node in
      let node = Node {
        size = size;
        priority = priority;
        left = left;
...
and nodeDelete compareKey dkey node =
  let Node {size; priority; left; key; value; right} = node in
  match compareKey dkey key with
  | Less ->
      let size = size - 1
      and left = treeDelete compareKey dkey left in
      let node = Node {size = size; priority = priority; left = left; key = key;
                       value = value; right = right} in
      Tree node
  | Equal -> treeAppend left right
  | Greater ->
      let size = size - 1
      and right = treeDelete compareKey dkey right in
      let node = Node {size = size; priority = priority; left = left; key = key;
                       value = value; right = right} in
      Tree node
;;
...
and nodeMapPartial f (Node {priority; left; key; value; right}) =
  ...
  and vo = f key value
  and right = treeMapPartial f right in
  match vo with
...
and nodeMap f node =
  ...
  and right = treeMap f right in
  Node {size = size; priority = priority; left = left; key = key; value = value;
        right = right}
;;
...
and nodeMerge compareKey f1 f2 fb node1 node2 =
  let Node {priority; left; key; value; right} = node2 in
  let (l,kvo,r) = nodePartition compareKey key node1 in
  let left = treeMerge compareKey f1 f2 fb l left
  and right = treeMerge compareKey f1 f2 fb r right in
  let vo = match kvo with
           | None -> f2 key value
           | Some kv -> fb kv (key,value) in
  match vo with
  | None -> treeAppend left right
  | Some value ->
      let node = mkNodeSingleton priority key value in
      treeCombine left node right
;;
...
and nodeUnion compareKey f f2 node1 node2 =
    if pointerEqual (node1,node2) then
      nodeMapPartial f2 node1
    else
      let Node {priority; left; key; value; right} = node2 in
      let (l,kvo,r) = nodePartition compareKey key node1 in
      let left = treeUnion compareKey f f2 l left
      and right = treeUnion compareKey f f2 r right in
      let vo = match kvo with
              | None -> Some value
              | Some kv -> f kv (key,value) in
      match vo with
      | None -> treeAppend left right
      | Some value ->
          let node = mkNodeSingleton priority key value in
...
and nodeIntersect compareKey f n1 n2 =
  let Node {priority; left; key; value; right} = n2 in
  let (l,kvo,r) = nodePartition compareKey key n1 in
  let left = treeIntersect compareKey f l left
  and right = treeIntersect compareKey f r right in
  let vo =
    match kvo with
    | None -> None
    | Some kv -> f (kv,(key,value)) in
...
and nodeUnionDomain compareKey node1 node2 =
  let Node {priority; left; key; value; right} = node2 in
  let (l,_,r) = nodePartition compareKey key node1 in
  let left = treeUnionDomain compareKey l left
  and right = treeUnionDomain compareKey r right in
  let node = mkNodeSingleton priority key value in
  treeCombine left node right
;;
...
and nodeIntersectDomain compareKey node1 node2 =
  let Node {priority; left; key; value; right} = node2 in
  let (l,kvo,r) = nodePartition compareKey key node1 in
  let left = treeIntersectDomain compareKey l left
  and right = treeIntersectDomain compareKey r right in
...
and nodeDifferenceDomain compareKey n1 n2 =
    if pointerEqual (n1,n2) then Empty
      ...
      let Node {priority; left; key; value; right} = n1 in
      let (l,kvo,r) = nodePartition compareKey key n2 in
      let left = treeDifferenceDomain compareKey left l
      and right = treeDifferenceDomain compareKey right r in
...
and nodeSubsetDomain compareKey node1 node2 =
  ...
  let Node {size; left; key; right} = node1 in
  size <= nodeSize node2 &&
  let (l,kvo,r) = nodePartition compareKey key node2 in
  Option.isSome kvo &&
...
and nodeDeleteNth n node =
  ...
  if n = k then ((key,value), treeAppend left right)
    ...
    let (key_value,left) = treeDeleteNth n left in
    let size = size - 1 in
    let node = Node {size = size; priority = priority; left = left; key = key;
                     value = value; right = right} in
    (key_value, Tree node)
    ...
    let n = n - (k + 1) in
    let (key_value,right) = treeDeleteNth n right in
    let size = size - 1 in
    let node = Node {size = size; priority = priority; left = left; key = key;
                     value = value; right = right} in
    (key_value, Tree node)
...
let fromSpineLeftToRightIterator nodes =
  ...
  | [] -> None
  | node :: nodes ->
      let Node {key; value; right} = node in
...
let fromSpineRightToLeftIterator nodes =
    ...
    | [] -> None
    | node :: nodes ->
        let Node {key; value; left} = node in
        Some (Right_to_left_iterator ((key,value),left,nodes));;

let addLeftToRightIterator nodes tree =
  fromSpineLeftToRightIterator (treeLeftSpine nodes tree);;

let addRightToLeftIterator nodes tree =
  fromSpineRightToLeftIterator (treeRightSpine nodes tree);;
...
let rec foldIterator f acc io =
  ...
  | None -> acc
  | Some iter ->
      let (key,value) = readIterator iter in
...
let rec findIterator pred io =
  ...
  | None -> None
  | Some iter ->
      let key_value = readIterator iter in
...
type ('key,'value) map =
  Map of ('key -> 'key -> ordering) * ('key,'value) tree;;
...
let singleton compareKey key_value =
  let tree = treeSingleton key_value in
  Map (compareKey,tree)
;;
...
let insert (Map (compareKey,tree)) key_value =
  let tree = treeInsert compareKey key_value tree in
  Map (compareKey,tree)
;;

let insertList m =
  let ins key_value acc = insert acc key_value in
  List.foldl ins m
;;
...
let delete (Map (compareKey,tree)) dkey =
  let tree = treeDelete compareKey dkey tree in
  Map (compareKey,tree)
;;
...
let deletePick (Map (compareKey,tree)) =
  let (key_value,tree) = treeDeletePick tree in
  (key_value, Map (compareKey,tree))
;;

let deleteNth (Map (compareKey,tree)) n =
  let (key_value,tree) = treeDeleteNth n tree in
  (key_value, Map (compareKey,tree))
;;
...
let merge (first,second,both) (Map (compareKey,tree1)) (Map (_,tree2)) =
  let tree = treeMerge compareKey first second both tree1 tree2 in
  Map (compareKey,tree)
;;

let union f (Map (compareKey,tree1)) (Map (_,tree2)) =
  let f2 k v = f (k,v) (k,v) in
  let tree = treeUnion compareKey f f2 tree1 tree2 in
  Map (compareKey,tree)
;;

let intersect f (Map (compareKey,tree1)) (Map (_,tree2)) =
  let tree = treeIntersect compareKey f tree1 tree2 in
  Map (compareKey,tree)
;;
...
let mapPartial f (Map (compareKey,tree)) =
  let tree = treeMapPartial f tree in
  Map (compareKey,tree)
;;

let map f (Map (compareKey,tree)) =
  let tree = treeMap f tree in
  Map (compareKey,tree)
;;
...
let filter pred =
  let f key value =
    if pred key value then Some value else None in
  mapPartial f
;;

let partition p =
  let np x y = not (p x y) in
...
let count pred =
  let f (k,v,acc) = if pred (k,v) then acc + 1 else acc in
  foldl f 0
;;
...
let compare compareValue m1 m2 =
  if pointerEqual (m1,m2) then Equal
    ...
    | Less -> Less
    | Equal ->
        let Map (compareKey,_) = m1 in
        let io1 = mkIterator m1
        and io2 = mkIterator m2 in
        compareIterator compareKey compareValue io1 io2
    | Greater -> Greater
;;
...
let unionDomain (Map (compareKey,tree1)) (Map (_,tree2)) =
  let tree = treeUnionDomain compareKey tree1 tree2 in
  Map (compareKey,tree)
;;
...
let intersectDomain (Map (compareKey,tree1)) (Map (_,tree2)) =
  let tree = treeIntersectDomain compareKey tree1 tree2 in
  Map (compareKey,tree)
;;
...
let differenceDomain (Map (compareKey,tree1)) (Map (_,tree2)) =
  let tree = treeDifferenceDomain compareKey tree1 tree2 in
  Map (compareKey,tree)
;;
