...
type reductionOrder = Term.term * Term.term -> ordering option;;
...
type rewrite = Rewrite of {
  ...
  known : (int, equation * orient option) Mmap.map;
  redexes : (equationId * orient) Term_net.termNet;
  subterms : (equationId * bool * Term.path) Term_net.termNet;
  waiting : int Mset.set
};;

let updateWaiting rw waiting =
  Rewrite {rw with waiting = waiting};;

let deleteWaiting rw id =
  Rewrite {rw with waiting = Intset.delete rw.Rewrite.waiting id};;
...
let orderToOrient = function
  ...
  | Some ord ->
      match ord with
      | Equal -> raise (Error "Rewrite.orient: reflexive")
      | Greater -> Some Left_to_right
      | Less -> Some Right_to_left;;
...
let addRedexes id (((l,r),_),ort) redexes =
  ...
  | Some ort ->
      match ort with
      | Left_to_right -> ins redexes l id Left_to_right
      | Right_to_left -> ins redexes r id Right_to_left
;;

let add rw (id,eqn) =
  let Rewrite {known} = rw in
  if Intmap.inDomain id known then rw else
    let Rewrite {order; redexes; subterms; waiting} = rw in
    let ort = orderToOrient (order (fst eqn)) in
    let known = Intmap.insert known (id,(eqn,ort)) in
    let redexes = addRedexes id (eqn,ort) redexes in
    let waiting = Intset.add waiting id in
    Rewrite {
...
let addList rw = List.foldl (curry uncurriedAdd) rw;;
...
let wellOriented x y = match (x,y) with
  | (None, _) -> true
  (* FIXME Something is wrong with nested pattern parsing:
   * This used to be | Some Left_to_right, Left_to_right -> ...
   *                 | Some Right_to_left, Right_to_left -> ...
   *)
  | (Some z, w) -> z = w
  | _ -> false;;
...
let rewrIdConv' order known redexes id tm =
  let rewr (id',lr) =
    let _ = id <> id' || raise (Error "same theorem") in
    let (eqn,ort) = Intmap.get known id' in
    let _ = wellOriented ort lr || raise (Error "orientation") in
    let (l,r) = redexResidue lr eqn in
    let sub =
      Substitute.normalize (Substitute.matchTerms Substitute.empty l tm) in
    let tm' = Substitute.subst sub r in
    let _ = Option.isSome ort ||
            order (tm,tm') = Some Greater ||
            raise (Error "order") in
    let (_,th) = orientedEquation lr eqn in
    (tm', Thm.subst sub th) in
...
let mkNeqConv order lit =
  let (l,r) = Literal.destNeq lit in
  match order (l,r) with
  | None -> raise (Error "incomparable")
  | Some ord ->
      match ord with
      | Less ->
          let th = Rule.symmetryRule l r in
          fun tm ->
            if Term.equal tm r then (l,th) else raise (Error "mkNeqConv: RL")
      | Equal -> raise (Error "irreflexive")
      | Greater ->
          let th = Thm.assume lit in
          fun tm ->
            if Term.equal tm l then (r,th) else raise (Error "mkNeqConv: LR")
;;

type neqConvs = Neq_convs of (Literal.literal, Rule.conv) Mmap.map;;
...
let mkNeqConvs order =
  let add (lit,(neq,lits)) =
    ...
    | Some neq -> (neq,lits)
    | None -> (neq, Literal.Set.add lits lit) in
...
let neqConvsRewrIdLiterule order known redexes id neq =
  if Intmap.null known && neqConvsNull neq then Rule.allLiterule
    ...
    let neq_conv = neqConvsToConv neq in
    let rewr_conv = rewrIdConv' order known redexes id in
    let conv = Rule.orelseConv neq_conv rewr_conv in
    let conv = Rule.repeatTopDownConv conv in
    Rule.allArgumentsLiterule conv
;;

let rewriteIdEqn' order known redexes id ((l_r,th) as eqn) =
  let (neq,_) = mkNeqConvs order (Thm.clause th) in
  let literule = neqConvsRewrIdLiterule order known redexes id neq in
  let (strongEqn,lit) =
    match Rule.equationLiteral eqn with
    | None -> (true, Literal.mkEq l_r)
    | Some lit -> (false,lit) in
...
let rewriteIdLiteralsRule' order known redexes id lits th =
  let mk_literule = neqConvsRewrIdLiterule order known redexes id in
  let rewr_neq_lit (lit, ((changed,neq,lits,th) as acc)) =
    let neq = neqConvsDelete neq lit in
    let (lit',litTh) = mk_literule neq lit in
    if Literal.equal lit lit' then acc
    else
      let th = Thm.resolve lit th litTh in
      match neqConvsAdd order neq lit' with
      | Some neq -> (true,neq,lits,th)
      | None -> (changed, neq, Literal.Set.add lits lit', th) in
  let rec rewr_neq_lits neq lits th =
      let (changed,neq,lits,th) =
        neqConvsFoldl rewr_neq_lit (false,neq,lits,th) neq in
      if changed then rewr_neq_lits neq lits th
      else (neq,lits,th) in
  let (neq,lits) = mkNeqConvs order lits in
  let (neq,lits,th) = rewr_neq_lits neq lits th in
  let rewr_literule = mk_literule neq in
  let rewr_lit (lit,th) =
    if Thm.member lit th then Rule.literalRule rewr_literule lit th else th in
...
let addSubterms id (((l,r),_) : equation) subterms =
  let addSubterm b ((path,tm),net) = Term_net.insert net (tm,(id,b,path)) in
  let subterms =
    List.foldl (curry (addSubterm true)) subterms (Term.subterms l) in
  let subterms =
    List.foldl (curry (addSubterm false)) subterms (Term.subterms r) in
  subterms
;;
...
let sameRedexes x y z = match (x,y,z) with
  ...
  | (Some ort, (l0,r0),(l,r)) ->
      match ort with
      | Left_to_right -> Term.equal l0 l
      | Right_to_left -> Term.equal r0 r;;

let redexResidues x (l,r) = match x with
  | None -> [(l,r,false);(r,l,false)]
  | Some ort ->
      match ort with
      | Left_to_right -> [(l,r,true)]
      | Right_to_left -> [(r,l,true)];;

let findReducibles order known subterms id =
  let checkValidRewr (l,r,ord) id' left path =
    let (((x,y),_),_) = Intmap.get known id' in
    let tm = Term.subterm (if left then x else y) path in
    let sub = Substitute.matchTerms Substitute.empty l tm in
    if ord then ()
    else
      let tm' = Substitute.subst (Substitute.normalize sub) r in
      if order (tm,tm') = Some Greater then ()
      else raise (Error "order") in
  let addRed lr ((id',left,path),todo) =
    if id <> id' && not (Intset.member id' todo) &&
       can (checkValidRewr lr id' left) path
    then Intset.add todo id'
    else todo in
  let findRed ((l,_,_) as lr, todo) =
    List.foldl (curry (addRed lr)) todo (Term_net.matched subterms l) in
  List.foldl (curry findRed)
;;

let reduce1 newx id (eqn0,ort0) (rpl,spl,todo,rw,changed) =
  let (eq0,_) = eqn0 in
  let Rewrite {order; known; redexes; subterms; waiting} = rw in
  let (eq,_) as eqn = rewriteIdEqn' order known redexes id eqn0 in
  let identical =
    let (l0,r0) = eq0
    and (l,r) = eq in
    Term.equal l l0 && Term.equal r r0 in
  let same_redexes = identical || sameRedexes ort0 eq0 eq in
  let rpl = if same_redexes then rpl else Intset.add rpl id in
  let spl = if newx || identical then spl else Intset.add spl id in
  let changed =
    if not newx && identical then changed else Intset.add changed id in
  let ort =
    if same_redexes then Some ort0 else total orderToOrient (order eq) in
  match ort with
  | None ->
      let known = Intmap.delete known id in
      let rw = Rewrite {
        order = order; known = known; redexes = redexes;
  ...
  | Some ort ->
      let todo =
        if not newx && same_redexes then todo
          ...
          findReducibles order known subterms id todo (redexResidues ort eq) in
      let known =
        if identical then known else Intmap.insert known (id,(eqn,ort)) in
      let redexes = if same_redexes then redexes
                    else addRedexes id (eqn,ort) redexes in
      let subterms =
        if newx || not identical then addSubterms id eqn subterms
        else subterms in
      let rw =
        Rewrite {order = order; known = known; redexes = redexes;
                 subterms = subterms; waiting = waiting} in
      (rpl,spl,todo,rw,changed)
;;
...
let pick known set =
  let oriented id =
    ...
    | _ -> None in
  let any id =
    match Intmap.peek known id with Some x -> Some (id,x) | _ -> None in
...
let cleanRedexes known redexes rpl =
  if Intset.null rpl then redexes
    ...
    let filt (id,_) = not (Intset.member id rpl) in
    let addReds (id,reds) =
      match Intmap.peek known id with
      | None -> reds
      | Some eqn_ort -> addRedexes id eqn_ort reds in
    let redexes = Term_net.filter filt redexes in
    let redexes = Intset.foldl addReds redexes rpl in
    redexes
;;
...
let cleanSubterms known subterms spl =
  if Intset.null spl then subterms
    ...
    let filt (id,_,_) = not (Intset.member id spl) in
    let addSubtms (id,subtms) =
      match Intmap.peek known id with
      | None -> subtms
      | Some (eqn,_) -> addSubterms id eqn subtms in
    let subterms = Term_net.filter filt subterms in
    let subterms = Intset.foldl addSubtms subterms spl in
    subterms
;;

let rebuild rpl spl rw =
  let Rewrite {order; known; redexes; subterms; waiting} = rw in
  let redexes = cleanRedexes known redexes rpl in
  let subterms = cleanSubterms known subterms spl in
  Rewrite {
    order = order; known = known; redexes = redexes; subterms = subterms;
    waiting = waiting}
;;
...
let orderedRewrite order ths =
  let rw = List.foldl addEqn (newRewrite order) (enumerate ths) in
