...
(*                                                                           *)
(* This is a straight port of the old HOL88 wellorder library; the only real *)
(* difference is the recent switch to using curried inequalities.            *)
(* ========================================================================= *)
...
let FL_SUBSET = prove
 (`!l r. (!x y. l x y ==> r x y) ==> fl l SUBSET fl r`,
  REWRITE_TAC[SUBSET; IN; fl] THEN MESON_TAC[]);;

let FINITE_FL = prove
 (`!l:A->A->bool. FINITE(fl l) <=> FINITE {(x,y) | l x y}`,
  ...
  REWRITE_TAC[IN_CROSS; FL; IN_IMAGE; IN_ELIM_THM; IN_UNION] THEN
...
let UNION_FL = prove
 (`!P (l:A->A->bool). fl(\x y. ?l. P l /\ l x y) x <=> ?l. P l /\ fl(l) x`,
  REPEAT GEN_TAC THEN REWRITE_TAC[fl] THEN MESON_TAC[]);;
...
let ORDINAL_UNION_LEMMA = prove
 (`!(l:A->A->bool) x.
      ordinal l ==> fl(l) x ==> fl(\a b. ?l. ordinal l /\ l a b) x`,
...
let FL_RESTRICT = prove
 (`!l. woset l ==>
       !P. fl(\x y:A. P x /\ P y /\ l x y) x <=> P x /\ fl(l) x`,
...
let WO = prove
 (`!P. ?l:A->A->bool. woset l /\ (fl(l) = P)`,
  ...
  EXISTS_TAC `\x y:A. P x /\ P y /\ l x y` THEN REWRITE_TAC[WOSET] THEN
  PBETA_TAC THEN
...
let OEP = prove
 (`!p:A->A->bool.
     poset p ==> ?t. toset t /\ fl(t) = fl(p) /\ !x y. p x y ==> t x y`,
  REPEAT STRIP_TAC THEN
