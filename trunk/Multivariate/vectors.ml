...
let MATRIX_ADD_LDISTRIB = prove
 (`!A:real^N^M B:real^P^N C. A ** (B + C) = A ** B + A ** C`,
  SIMP_TAC[matrix_mul; matrix_add; CART_EQ; LAMBDA_BETA; GSYM SUM_ADD_NUMSEG] THEN
...
(* ------------------------------------------------------------------------- *)
(* Inverse matrices (not necessarily square, but that's the usual case).     *)
(* ------------------------------------------------------------------------- *)
...
let MATRIX_WORKS = prove
 (`!f:real^M->real^N. linear f ==> !x. matrix f ** x = f(x)`,
  ...
  ASM_SIMP_TAC[GSYM LINEAR_COMPONENTWISE]);;
...
let ISOMETRY_SUBSPACES = prove
 (`!s:real^M->bool t:real^N->bool.
  ...
  STRIP_TAC THEN
  MP_TAC(ISPECL [`f:real^M->real^N`; `b:real^M->bool`]
    LINEAR_INDEPENDENT_EXTEND) THEN
  ASM_REWRITE_TAC[] THEN MATCH_MP_TAC MONO_EXISTS THEN
  X_GEN_TAC `h:real^M->real^N` THEN
  STRIP_TAC THEN ASM_REWRITE_TAC[] THEN CONJ_TAC THENL
   [REWRITE_TAC[SYM(ASSUME `span(b:real^M->bool) = s`);
                SYM(ASSUME `span(c:real^N->bool) = t`)] THEN
    MATCH_MP_TAC SUBSET_ANTISYM THEN
    REWRITE_TAC[SUBSET; FORALL_IN_IMAGE] THEN
    CONJ_TAC THEN MATCH_MP_TAC SPAN_INDUCT THEN REWRITE_TAC[IN_ELIM_THM] THENL
     [GEN_REWRITE_TAC (RAND_CONV o RAND_CONV)
       [SET_RULE `(\x. p x) = {x | p x}`] THEN
      ASM_SIMP_TAC[SUBSPACE_LINEAR_PREIMAGE] THEN
      ASM_MESON_TAC[SPAN_INC; SUBSET];
      ONCE_REWRITE_TAC[SET_RULE `(\x. x IN s) = s`] THEN
      ASM_SIMP_TAC[SUBSPACE_LINEAR_IMAGE; SUBSPACE_SPAN] THEN
      X_GEN_TAC `x:real^N` THEN DISCH_TAC THEN REWRITE_TAC[IN_IMAGE] THEN
      ASM_MESON_TAC[SPAN_INC; SUBSET]];
    X_GEN_TAC `x:real^M` THEN
    REWRITE_TAC[SYM(ASSUME `span(b:real^M->bool) = s`)] THEN
    SIMP_TAC[SPAN_FINITE; ASSUME `FINITE(b:real^M->bool) /\ CARD b = n`] THEN
    REWRITE_TAC[IN_ELIM_THM; LEFT_IMP_EXISTS_THM] THEN
    X_GEN_TAC `u:real^M->real` THEN DISCH_THEN(SUBST1_TAC o SYM) THEN
    ASM_SIMP_TAC[LINEAR_VSUM; NORM_EQ] THEN
    ASM_SIMP_TAC[o_DEF; LINEAR_CMUL] THEN
    ASM_SIMP_TAC[DOT_LSUM; DOT_RSUM; DOT_RMUL; DOT_LMUL] THEN
    MATCH_MP_TAC SUM_EQ THEN X_GEN_TAC `x:real^M` THEN DISCH_TAC THEN
    REWRITE_TAC[] THEN MATCH_MP_TAC SUM_EQ THEN X_GEN_TAC `y:real^M` THEN
    DISCH_TAC THEN REWRITE_TAC[] THEN AP_TERM_TAC THEN AP_TERM_TAC THEN
    RULE_ASSUM_TAC(REWRITE_RULE[pairwise; orthogonal]) THEN
    ASM_CASES_TAC `x:real^M = y` THEN ASM_REWRITE_TAC[GSYM NORM_EQ] THEN
    ASM SET_TAC[]]);;
...
let DIM_IMAGE_KERNEL = prove
 (`!f:real^M->real^N.
  ...
  REPEAT STRIP_TAC THEN
  MP_TAC(ISPEC `{x | (f:real^M->real^N) x = vec 0}` BASIS_EXISTS) THEN
  DISCH_THEN(X_CHOOSE_THEN `v:real^M->bool` STRIP_ASSUME_TAC) THEN
  MP_TAC(ISPECL [`v:real^M->bool`; `(:real^M)`]
    MAXIMAL_INDEPENDENT_SUBSET_EXTEND) THEN
  ASM_REWRITE_TAC[SUBSET_UNIV; SET_RULE `UNIV SUBSET s <=> s = UNIV`] THEN
  DISCH_THEN(X_CHOOSE_THEN `w:real^M->bool` STRIP_ASSUME_TAC) THEN
  REWRITE_TAC[GSYM DIM_UNIV] THEN
  ASM (GEN_REWRITE_TAC (RAND_CONV o RAND_CONV) o map GSYM) [] THEN
  SUBGOAL_THEN `{x | (f:real^M->real^N) x = vec 0} = span v` ASSUME_TAC THENL
  ...
  (LABEL_TAC "*") THENL
   [ASM_SIMP_TAC[SPAN_FINITE; IN_ELIM_THM; IMP_CONJ; FINITE_DIFF;
    ...
    GEN_REWRITE_TAC LAND_CONV [SET_RULE `f x = a <=> x IN {x | f x = a}`] THEN
    ASM_REWRITE_TAC[] THEN
  ...
  ASSUME_TAC THENL
   [REMOVE_THEN "*" MP_TAC THEN
    ...
    ALL_TAC] THEN
  SUBGOAL_THEN `IMAGE (f:real^M->real^N) (:real^M) = span(IMAGE f (w DIFF v))`
  SUBST1_TAC THENL
   [MATCH_MP_TAC SUBSET_ANTISYM THEN
    ASM_SIMP_TAC[SPAN_SUBSET_SUBSPACE; IMAGE_SUBSET; SUBSET_UNIV;
                 SUBSPACE_LINEAR_IMAGE; SUBSPACE_UNIV] THEN
    SIMP_TAC[SUBSET; FORALL_IN_IMAGE; IN_UNIV] THEN X_GEN_TAC `x:real^M` THEN
    UNDISCH_TAC `span w = (:real^M)` THEN REWRITE_TAC[EXTENSION] THEN
    DISCH_THEN(MP_TAC o SPEC `x:real^M`) THEN REWRITE_TAC[IN_UNIV] THEN
