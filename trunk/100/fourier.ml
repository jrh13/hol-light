...
let lnorm = new_definition
 `lnorm s p f =
   drop(integral s (\x. lift(norm(f x) rpow p))) rpow (inv p)`;;
  ...
  ANTS_TAC THENL
   [REPEAT CONJ_TAC THENL
     [X_GEN_TAC `n:num` THEN
      ...
      MATCH_MP_TAC LIM_RPOW_NULL THEN
    ...
    DISCH_THEN(MP_TAC o MATCH_MP
     (REWRITE_RULE[IMP_CONJ; o_DEF] LIM_RPOW_NULL)) THEN
...
let L2NORM_LMUL = prove
 (`!f s c. f square_integrable_on s
  ...
  ASM_SIMP_TAC[REAL_MUL_ASSOC; SQRT_MUL; REAL_LE_SQUARE;
               L2PRODUCT_POS_LE] THEN
   ...
   [REPEAT CONJ_TAC THENL
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [MP_TAC(REAL_ARITH `&0 < &n + &1`) THEN
      ...
      REWRITE_TAC[GSYM REAL_POW_2] THEN CONJ_TAC THENL
...
let FOURIER_SUM_0_EXPLICIT = prove
 (`!f n.
  ...
  BINOP_TAC THENL
   [REWRITE_TAC[REAL_MUL_LID; GSYM REAL_INV_MUL; GSYM REAL_MUL_ASSOC] THEN
    AP_TERM_TAC THEN AP_TERM_TAC THEN MATCH_MP_TAC SQRT_MUL THEN
    MP_TAC PI_POS THEN REAL_ARITH_TAC;
