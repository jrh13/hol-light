...
let SOLUTIONS_INDUCTION = prove
 (`!a x y.
  ...
  SUBGOAL_THEN `x <= a * y:num` ASSUME_TAC THENL
   [ONCE_REWRITE_TAC[SYM(REWRITE_RULE[NOT_SUC]
      (SPECL [`x:num`; `y:num`; `SUC 1`] EXP_MONO_LE))] THEN
  ...
  CONJ_TAC THENL
   [GEN_REWRITE_TAC LAND_CONV 
     [SYM(REWRITE_RULE[NOT_SUC] 
      (SPECL [`x:num`; `y:num`; `SUC 1`] EXP_MONO_LT))] THEN
    ASM_REWRITE_TAC[ARITH_SUC] THEN
    REWRITE_TAC[LT_ADD_RCANCEL; LT_MULT_LCANCEL] THEN
    REWRITE_TAC[num_CONV `2`; EXP_MONO_LT; NOT_SUC] THEN
 ...
 (`!a k n.
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[LE_MULT_LCANCEL] THEN DISJ2_TAC THEN
    ...
    REWRITE_TAC[EXP_MONO_LE; NOT_SUC] THEN
    UNDISCH_TAC `~(a = 0)` THEN ARITH_TAC;
    ALL_TAC] THEN
 ...
 (`~(a = 0)
  ...
  EQ_TAC THENL
   [DISCH_THEN(SUBST1_TAC o SYM) THEN
    ...
    REWRITE_TAC[CONJUNCT1 EXP; LE_REFL; EXP_MONO_LE; NOT_SUC] THEN
    UNDISCH_TAC `~(a = 0)` THEN ARITH_TAC; ALL_TAC] THEN
